...
def main(sell):
    ...
    if sell.find('\x02') != 0 or sell.find('\x03') != len(sell) - 1:
        ...
        if cal_checkSum != checkSum:
            ...
            current_ticket = int(classdb.getTicket())  # Culsulta el ultimo ticket registrado
	    ...
	    "'" + str(class_db.idpanelServices()) + "', " \
            ...
            if existChange:
		classdb.recordSale(panelservices)
                classdb.recordSale(panelcat)
                classdb.recordSale(servicesdetail)
                classdb.recordSale(config)
